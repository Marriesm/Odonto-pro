            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <ScrollArea className='h-[340px] lg:max-h-[calc(100vh-15rem)] pr-0 w-full flex-1'>
                        {reminder.map((item) => (
                            <article key={item.id} className='flex flex-wrap items-center justify-between py-2 bg-yellow-200 mb-2 px-2 rounded-md'>
                                <p className='text-sm lg:text-base'>{item.description}</p>
                                <Button className='bg-red-700 hover:bg-red-500 shadow-none rounded-full p-2 ' size="sm"
                                    onClick={() => handleDeleteReminder(item.id)}>
                                    <Trash className='w-4 h-5 text-white' />
                                </Button>
                            </article>
                        ))}
                    </ScrollArea>
