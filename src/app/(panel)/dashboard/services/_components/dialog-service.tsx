...
export function DialogService({ closeModal, initialValues, serviceId }: DialogServiceProps) {
    ...
    async function onSubmit(values: DialogServiceFormData) {
        ...
        if (serviceId) {
            await handleEditService({ serviceId: serviceId, name: values.name, priceInCents: priceInCents, duration: duration })
            setLoading(false);
            return;
        }
        ...
        if (!response || response.error) {
            toast.error(response?.error ?? "Erro inesperado ao criar o serviço.");
            return;
        }
    ...
    async function handleEditService({ serviceId, name, priceInCents, duration }: { serviceId: string, name: string, priceInCents: number, duration: number }) {
